...
const Progress = () => {
  ...
  const scatter3DData = useMemo(() => {
    return lessonsProgress.slice(0, 20).map((lesson, i) => ({
      x: lesson.score || 0,
      y: lesson.time_spent,
      z: i * 5,
      label: lesson.lesson_id.replace(/-/g, ' ')
    }));
  }, [lessonsProgress]);
        ...
        <TabsContent value="grades" className="space-y-6">
          <Card className="border-border/40">
            <CardHeader>
              ...
              <CardDescription>Grades synced from your school portal</CardDescription>
            </CardHeader>
            <CardContent>
              {syncedGrades.length > 0 ? (
                <div className="space-y-3">
                  {syncedGrades.map((grade) => (
                    <div key={grade.id} className="flex items-center justify-between p-4 rounded-lg border bg-card">
                      <div>
                      ...
                      <div className="text-right">
                        <Badge 
                          variant={grade.grade >= 8 ? "default" : grade.grade >= 6 ? "secondary" : "destructive"}
                          className="text-lg px-3 py-1"
                        >
                          {grade.grade}
                        </Badge>
                        <p className="text-xs text-muted-foreground mt-1">
                          {formatDate(grade.date)} • {grade.source.toUpperCase()}
                        </p>
                      </div>
                    </div>
                  ))}
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-muted-foreground mb-4">No grades synced yet</p>
                  <p className="text-sm text-muted-foreground">
                    Save your Tamo or ManoDienynas credentials in Settings, then click Sync
                  </p>
                </div>
              )}
            </CardContent>
